import keyMirror from 'keymirror';

export default keyMirror({
    LOGOUT: null,
    LOGIN_PENDING: null,
    LOGIN_SUCCEED: null,
    LOGIN_FAILED: null,
    REGISTER_PENDING: null,
    REGISTER_SUCCEED: null,
    REGISTER_FAILED: null,
    LOGIN_FORM: null,
    LOGIN_FORM_FIELD: null,
    LOGIN_FORM_DELETE_ERRORS: null,
    LOGIN_FORM_ENABLED: null,
    LOGIN_FORM_REFRESH: null,
    REGISTER_FORM: null,
    REGISTER_FORM_FIELD: null,
    REGISTER_FORM_DELETE_ERRORS: null,
    REGISTER_FORM_ENABLED: null,
    REGISTER_FORM_REFRESH: null,
    USER_INFO_PENDING: null,
    USER_INFO_SUCCEED: null,
    USER_INFO_FAILED: null,
    TRANSACTION_FORM: null,
    TRANSACTION_FORM_FIELD: null,
    TRANSACTION_FORM_REFRESH: null,
    TRANSACTION_FORM_FILTERED_USERS: null,
    TRANSACTION_FORM_DELETE_ERRORS: null,
    TRANSACTION_FORM_ENABLED: null,
    TRANSACTION_PENDING: null,
    TRANSACTION_FAILED: null,
    TRANSACTION_SUCCEED: null,
    ROUTER_STATE_CHANGE: null,
    TRANSACTION_HISTORY_PENDING: null,
    TRANSACTION_HISTORY_SUCCEED: null,
    TRANSACTION_HISTORY_FAILED: null,
    SELECT_TRANSACTION: null
});